...
ggplot() +
  ...
  labs(title = "Tweet Location During Hurricane Dorian") +
  theme(
    plot.title = element_text(hjust = 0.5),
    axis.title.x = element_blank(),
    axis.title.y = element_blank()
...
counties <- counties %>% 
  mutate(dorrate_fake = if_else(dorrate == 0, 0.1, dorrate)) %>% 
  mutate(dorrate_log = log(dorrate_fake))
    
  
ggplot() +
  geom_sf(data = counties, 
    aes(fill = cut_interval(dorrate_log, 5)), color = "grey", lwd = 0.1, alpha = 0.8) +
